...
# LISTEN_PORT_HTTPS=${LISTEN_PORT_HTTPS:-8080}
# LISTEN_PORT_HTTP=${LISTEN_PORT_HTTP:-8080}
LISTEN_PORT_HTTPS=${LISTEN_PORT_HTTPS:-443}
LISTEN_PORT_HTTP=${LISTEN_PORT_HTTP:-80}
...
cat /rdk/node/envoy/envoy.https.tmpl.yaml | \
/bin/sed -e "s/\$LISTEN_PORT_HTTPS/$LISTEN_PORT_HTTPS/g" | \
/bin/sed -e "s/\$LISTEN_PORT_HTTP/$LISTEN_PORT_HTTP/g" | \
...
> /rdk/node/envoy/envoy.yaml

cat /rdk/node/envoy/envoy.yaml

/usr/local/bin/envoy -c /rdk/node/envoy/envoy.yaml